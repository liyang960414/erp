...
auto_attached:
  - path: src/**/*.vue
    prompt_modifier: "Use Element Plus components (<el-button>, <el-form>); avoid native HTML UI. Serve all UI text through $t()/t(); sync Element Plus locale with default zh-CN. Target desktop >=1024px with flex layouts, keep root/main overflow hidden, and allow scroll only in the content area. Pair every <el-table> with <el-pagination> and add max-height or a scroll wrapper when data is large."
  - path: src/locales/**/*.ts
    prompt_modifier: "Provide zh-CN, vi, en, id translations and keep key structure aligned across files."