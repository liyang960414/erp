<template>
  <div class="order-container">
      ...
      <template #header>
        <div class="card-header">
          ...
          <div class="header-actions">
            <el-button v-if="authStore.hasPermission('purchase_order:import')" type="success" @click="handleImport">
    ...
    <!-- 导入对话框 -->
    <PurchaseOrderImportDialog v-model="importDialogVisible" @success="handleImportSuccess" />
...
const orderDetailsMap = ref<Record<number, PurchaseOrder>>({})
...
const handleTableExpand = (row: PurchaseOrder, expandedRowsList: PurchaseOrder[]) => {
  // 展开行
    ...
    if (!orderDetailsMap.value[row.id] && !loadingDetails.value[row.id]) {
      ...
      purchaseOrderApi
        ...
        .catch((error: any) => {
          ...
          orderDetailsMap.value[row.id] = { ...row, items: [] } as PurchaseOrder
        })
        .finally(() => {
...
const handleViewDetail = async (order: PurchaseOrder) => {
  try {
    currentOrder.value = await purchaseOrderApi.getPurchaseOrderById(order.id)
