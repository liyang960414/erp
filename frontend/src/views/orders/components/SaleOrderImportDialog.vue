...
const handleImport = async () => {
  ...
  try {
    ...
    if (result.data.saleOrderResult.failureCount === 0) {
      ElMessage.success(
        `导入成功！共导入 ${result.data.saleOrderResult.successCount} 条订单`,
      )
      emit('success')
      setTimeout(() => {
        handleClose()
      ...
      ElMessage.warning(
        `导入完成！成功 ${result.data.saleOrderResult.successCount} 条，失败 ${result.data.saleOrderResult.failureCount} 条`,
      )
    }
  } catch (error: any) {
    progress.value = 0
